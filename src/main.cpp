...
#if defined(NDEBUG)
# error "Snipcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0731d3bc97a50b0dfa178e01470062557c4544aeff609cf42707ec8318ef21cc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Snipcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Snipcoin Signed Message:\n";
...
{
    // Snipcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 200000); // Snipcoin: 200k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Snipcoin: 3.5 days // 1 day
static const int64 nTargetSpacing = 0.5 * 60; // Snipcoin: 2.5 minutes // 30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Snipcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Snipcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1e3b64ae4bd27a718e585a49905203f78f9b4a66f65f7fe608f531a10d8fa493");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Block timestamp June 2020";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cda620cc862cdb9913b75a0b7e78bdac5206db40418922d710a1bc36b3e72bbdf10b4f1807a461eac91ecf2a36c2a53d01e2e031d5384cfdb873e854f0e07025") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1592492032;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085215688;

        if (fTestNet)
        {
            block.nTime    = 1592491992;
            block.nNonce   = 386348670;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd06161e8208db9aca5be38092a15212d7d0a175c584ea1f0f8b85abac8578a8a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xbf, 0xd4 }; // Snipcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SnipcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SnipcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SnipcoinMiner(CWallet *pwallet)
{
    printf("SnipcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("snipcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SnipcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SnipcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SnipcoinMiner, pwallet));
}
